...
pub trait Controller: Send + Sync {
    ...
    /// This operation requires validation that the broker is offline unless
    /// `clean_living_broker` is explicitly set to true.
    ///
    /// # Arguments
    ///
    /// * `request` - The clean broker data request containing cluster name, broker name,
    ///               optional broker IDs to clean, and safety flags
    ///
    /// # Returns
    ///
